...
# === 主程序函数 ===
def plot_constellation_without_phase_tracking(filename, num_symbols):
    ...
    plt.title(f"星座图（{filename}, 未进行相位追踪）")
    plt.xlabel("实部")
    plt.ylabel("虚部")
    plt.grid(True)
    plt.axis("equal")
...
# === 执行部分 ===
plot_constellation_without_phase_tracking("recorded_signal_10sym.npy", num_symbols=10)
plot_constellation_without_phase_tracking("recorded_signal_100sym.npy", num_symbols=100)
