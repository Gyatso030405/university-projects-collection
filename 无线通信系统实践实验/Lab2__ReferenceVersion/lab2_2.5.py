...
def bpsk_demodulation( h_tilde, sym_num, data_index,signal,pilot_index,pilot_value):
    ...
    print("Demodulation Successfully!")
    return demod
...
def phase_track(pilot_signal_freq,pilot_value,pilot_index):
    ...
    compensate = fftshift(compensate)
    return compensate
...
signal = np.load("recorded_signal_10sym.npy")
...
if __name__ == '__main__':
    ...
    h_tilde_CPcut_under_CFO_estimation = channel_estimation(signal_lts_new,index,lts_frequency)
    
    channel_amplitude_CPcut_under_CFO_estimation = fftshift(h_tilde_CPcut_under_CFO_estimation)
    x = np.arange(-32,32)
    plt.plot(x,abs(channel_amplitude_CPcut_under_CFO_estimation))
    plt.xlabel('Freq')
    plt.ylabel('Amplitude')
    plt.title("channel_CPcut_under_CFO_estimation")
    plt.show()

    channel_angle_CPcut_under_CFO_estimation = np.angle(channel_amplitude_CPcut_under_CFO_estimation)
    x = np.arange(-32,32)
    plt.plot(x,abs(channel_angle_CPcut_under_CFO_estimation))
    plt.xlabel('Freq')
    plt.ylabel('Angle')
    plt.title("channel_angle_CPcut_under_CFO_estimation")
    plt.show()
    signal_data = signal[start_index[0] + 128 : start_index[0] + 128 + symbol_num * ( N + cp_len)]
    demod_signal = bpsk_demodulation( h_tilde_CPcut_under_CFO_estimation, symbol_num, data_index,signal_data,pilot_index,pilot_value)
    plt.scatter(np.array(demod_signal).real, np.array(demod_signal).imag, marker='.')
    plt.title('planisphere_CPcut_under_CFO_estimation')
    plt.show()

    #Cut_2
    
    for i in range(start_index_raw[0],start_index_raw[0]+128+symbol_num*80):
        signal[i] = signal[i] * np.exp(-1j*2*np.pi*cfo*(i-222))
    #% channel estimation by preamble
    signal_lts_new = signal[start_index_raw[0]:start_index_raw[0]+128]
    h_tilde_under_channle_estimation = channel_estimation(signal_lts_new,index,lts_frequency)
    
    channel_amplitude_under_channle_estimation = fftshift(h_tilde_under_channle_estimation)
    x = np.arange(-32,32)
    plt.plot(x,abs(channel_amplitude_under_channle_estimation))
    plt.xlabel('Freq')
    plt.ylabel('Amplitude')
    plt.title("channel_CPcut_under_channle_estimation")
    plt.show()

    channel_angle_under_channle_estimation = np.angle(channel_amplitude_under_channle_estimation)
    x = np.arange(-32,32)
    plt.plot(x,abs(channel_angle_under_channle_estimation))
    plt.xlabel('Freq')
    plt.ylabel('Angle')
    plt.title("channel_angle_CPcut_under_channle_estimation")
    plt.show()

    start_index  += cp_cut
    signal_data = signal[start_index[0] + 128 : start_index[0] + 128 + symbol_num * ( N + cp_len)]
    demod_signal = bpsk_demodulation( h_tilde_under_channle_estimation, symbol_num, data_index,signal_data,pilot_index,pilot_value)
    plt.scatter(np.array(demod_signal).real, np.array(demod_signal).imag, marker='.')
    plt.title('planisphere_under_channle_estimation')
    plt.show()


    #phase_difference
    x = np.arange(-32,32)
    plt.plot(x,abs(channel_angle_CPcut_under_CFO_estimation)-abs(channel_angle_under_channle_estimation))
