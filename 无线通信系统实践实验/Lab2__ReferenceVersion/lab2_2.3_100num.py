...
def bpsk_demodulation( h_tilde, sym_num, data_index,signal,pilot_index):
    ...
    for symi in range(0,sym_num):
        ...
        signal_data_no_STO_and_CFO.append(signal_freq/h_temp)
    ...
    plt.scatter(np.array(signal_data_no_STO_and_CFO).real, np.array(signal_data_no_STO_and_CFO).imag, marker='.')
    plt.title('planisphere_no_5.2-5.4')
    plt.show()
