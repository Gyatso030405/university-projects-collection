...
def bpsk_demodulation( h_tilde, sym_num, data_index,signal,pilot_index,pilot_value):
    ...
    for symi in range(0,sym_num):
        ...
        angle.append(np.angle(pilot_temp[0]))
        if (symi == 0):
            subcarrier_phase_1  = np.angle(pilot_temp)
        if (symi == 9):
            subcarrier_phase_10 = np.angle(pilot_temp)
        if (symi == 19):
            subcarrier_phase_20 = np.angle(pilot_temp)
    x = np.arange(0,100)
    plt.plot(x,np.unwrap(angle))
    plt.xlabel("symbol_num")
    plt.ylabel("Angle")
    plt.title('pilot_angle')
    plt.show()
    x = [-21, -7, 7, 21]
    y1 = np.interp(np.arange(-32,32), x, subcarrier_phase_1)
    x_all = np.arange(-32,32)
    plt.plot(x, subcarrier_phase_1, '-o')
    plt.plot(x_all, y1, 'x')
    plt.xlabel("symbol_num")
    plt.ylabel("Angle")
    plt.title('subcarrier_phase_1')
    plt.show()

    y2 = np.interp(np.arange(-32,32), x, subcarrier_phase_10)
    plt.plot(x, subcarrier_phase_10, '-o')
    plt.plot(x_all, y2, 'x')
    plt.xlabel("symbol_num")
    plt.ylabel("Angle")
    plt.title('subcarrier_phase_10')
    plt.show()

    y3 = np.interp(np.arange(-32,32), x, subcarrier_phase_20)
    plt.plot(x, subcarrier_phase_20, '-o')
    plt.plot(x_all, y3, 'x')
    plt.xlabel("symbol_num")
    plt.ylabel("Angle")
    plt.title('subcarrier_phase_20')
    plt.show()
...
signal = np.load("recorded_signal_100sym.npy")
...
if __name__ == '__main__':
    ...
    channel_amplitude = fftshift(h_tilde)
    x = np.arange(-32,32)
    plt.plot(x,abs(channel_amplitude))
    plt.title("channel")
    plt.show()
