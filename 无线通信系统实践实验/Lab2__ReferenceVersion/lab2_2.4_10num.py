...
def bpsk_demodulation( h_tilde, sym_num, data_index,signal,pilot_index):
    ...
    for symi in range(0,sym_num):
        ...
        signal_data_no_phase_tracking.append(signal_freq/h_temp)
    ...
    plt.scatter(np.array(signal_data_no_phase_tracking).real, np.array(signal_data_no_phase_tracking).imag, marker='.')
    plt.title('planisphere_no_2_and_5.2-5.4')
    plt.show()
    return demod
...
def phase_track(pilot_signal_freq,pilot_value,pilot_index):
    ...
    compensate = np.exp(-1j*(np.arange(-32, 32) * A1+B1))
    return np.concatenate((compensate[32:], compensate[0:32]), axis=0)

def phase_track_noSTO(pilot_signal_freq,pilot_value, pilot_index):
    pilot_angle = np.angle(pilot_value / pilot_signal_freq)
    return np.exp(np.sum(pilot_angle) / len(pilot_index))
